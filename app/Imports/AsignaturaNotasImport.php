                ...
                if($carrera && $asignatura && $turno)         // Si carrera, asignatura y turno estan definidos (se encontro valores)
                {
                    ...
                    if($carrera->id == $asignatura->carrera_id)     // Si el id de carrera en la tabla carrera es igual a carrera_id en la tabla asignaturas
                    {
                        ...
                        if($row[7] == 'Semestral')          // Si es Semestral
                        {
                            ...
                            $totalUno = $row[11] + $row[12] + $row[13] + $row[14];
                            $necesario = 100 - $totalUno;
                            if($necesario >= 10)
                            {
                                $totalUno = $totalUno + $row[15];
                            }
                            else
                            {
                                if($necesario <= $row[15])
                            ...
                            $notaUno->paralelo              = $row[9];
                            $notaUno->anio_vigente          = $row[10];
                            $notaUno->trimestre             = 1;
                            $notaUno->fecha_registro        = date('Y-m-d');
                            $notaUno->nota_asistencia       = $row[11];
                            $notaUno->nota_practicas        = $row[12];
                            $notaUno->nota_primer_parcial   = $row[13];
                            $notaUno->nota_examen_final     = $row[14];
                            $notaUno->nota_puntos_ganados   = $row[15];
                            ...
                            $notaDos->paralelo              = $row[9];
                            $notaDos->anio_vigente          = $row[10];
                            $notaDos->trimestre             = 2;
                            $notaDos->fecha_registro        = date('Y-m-d');
                            $notaDos->nota_asistencia       = $row[16];
                            $notaDos->nota_practicas        = $row[17];
                            $notaDos->nota_primer_parcial   = $row[18];
                            $notaDos->nota_examen_final     = $row[19];
                            $notaDos->nota_puntos_ganados   = $row[20];
                            ...
                            $notaTres->paralelo              = $row[9];
                            $notaTres->anio_vigente          = $row[10];
                            ...
                            $notaCuatro->paralelo              = $row[9];
                            $notaCuatro->anio_vigente          = $row[10];
                        ...
                        if($row[7] == 'Anual')              // Si es Anual
                        {
                            ...
                            $notaTres->trimestre             = 3;
                            $notaTres->fecha_registro        = date('Y-m-d');
                            $notaTres->nota_asistencia       = $row[21];
                            $notaTres->nota_practicas        = $row[22];
                            $notaTres->nota_primer_parcial   = $row[23];
                            $notaTres->nota_examen_final     = $row[24];
                            $notaTres->nota_puntos_ganados   = $row[25];
                            ...
                            if($necesario >= 10)
                            {
                                $totalCuatro = $totalCuatro + $row[30];
                            }
                            else
                            {
                                if($necesario <= $row[30])
                            ...
                            $notaCuatro->trimestre             = 4;
                            $notaCuatro->fecha_registro        = date('Y-m-d');
                            $notaCuatro->nota_asistencia       = $row[26];
                            $notaCuatro->nota_practicas        = $row[27];
                            $notaCuatro->nota_primer_parcial   = $row[28];
                            $notaCuatro->nota_examen_final     = $row[29];
                            $notaCuatro->nota_puntos_ganados   = $row[30];
                        ...
                        if(!$row[31] || $row[31] == 'No')       // Si la casilla de convalidado esta vacia, o si esta con el valor No
                        {
                        ...
                        {
                            $inscripcion->nota      = $row[33];
                            ($row[33] >= $inscripcion->nota_aprobacion ? $aprobo = 'Si' : $aprobo = 'No');
                            $inscripcion->aprobo    = $aprobo;
                            $inscripcion->estado    = 'Finalizado';
                            $inscripcion->save();
                        }
