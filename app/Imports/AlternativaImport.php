...
class AlternativaImport implements ToModel
{
    /**
    * @param array $row
    *
            ...
            if($persona)         // Si existe un registro, procedemos
            {
                ...
                if($carrera && $asignatura && $turno)         // Si carrera, asignatura y turno estan definidos (se encontro valores)
                {
                    ...
                    if($carrera->id == $asignatura->carrera_id)     // Si el id de carrera en la tabla carrera es igual a carrera_id en la tabla asignaturas
                    {
                        ...
                        // Si encuentra una cedula de identidad, automaticamente, eliminara y volvera a crear sus registros 
                        // En carreras_personas, inscripciones, notas
                        // $carrerasPersona    = CarrerasPersona::where('persona_id', $persona->id)
                        //                                 ->get();
                        // $inscripciones      = Inscripcione::where('persona_id', $persona->id)
                        //                                 ->get();
                        // $notas              = Nota::where('persona_id', $persona->id)
                        //                                 ->get();
                        // // Eliminamos registros
                        // foreach($carrerasPersona as $carreraPersona)
                        // {
